...
class Event(ApiComponent, AttachableMixin, HandleRecipientsMixin):
    ...
    _endpoints = {
        ...
        'event_attachment': '/events/{id}/attachments',
        'delete_attachment': '/events/{id}/attachments/{attach_id}',
    }
    ...
    def __init__(self, *, parent=None, con=None, **kwargs):
        ...
        self.attachment_obj_id = cloud_data.get(cc('attachment_obj_id'), None)
        ...
        self.__attachment_detail = self.__attachments
        self.__attachment_obj = self.__attachments
    ...
    def to_api_data(self, restrict_keys=None):
        ...
        if self.has_attachments:
            ...
            data[cc('attachment_detail')] = self.__attachment_detail.detail_to_api_data()
        if restrict_keys:
    ...
    def deleteattachment(self):
        ...
        if self.object_id is None:
            raise RuntimeError('Attempting to delete an unsaved event')
        if self.attachment_obj_id is None:
            raise RuntimeError('Attempting to delete an unsaved event')
        url = self.build_url(
            self._endpoints.get('delete_attachment').format(id=self.object_id, attach_id=self.attachment_obj_id))
    ...
    def save(self):
        ...
        _attachments = data.get('attachmentDetail', None)
        if self.object_id:
            # update event
            ...
            data = self.to_api_data()
        data.pop('attachmentDetail', None)
        print("url", url)
        response = method(url, data=data)
        at_response = []
        if response:
            event = response.json()
            _object_id = event.get(self._cc('id'), None)
        # data.pop('attachments', None)
        attach_url = self.build_url(
            self._endpoints.get('event_attachment').format(id=_object_id))
        if _attachments is not None and _object_id:
            url = self.build_url(
                self._endpoints.get('event_attachment').format(id=_object_id))
            method = self.con.post
            for d in _attachments:
                try:
                    response = method(url, data=d['attachment'])
                    # method = self.con.post
                    # response = method(attach_url, data=data)
                    at = response.json()
                    attachment = at.get(self._cc('id'), None)
                    at_detail = {'id': d['id'], 'obj_id': attachment}
                    at_response.append(at_detail)
                except:
                    continue
            _url = self.build_url(
                self._endpoints.get('event').format(id=_object_id))
            method = self.con.patch
            response = method(_url, data=self.to_api_data(restrict_keys=self._track_changes))
        if not response:
            return False
        ...
        self.__attachment_response = at_response
        return True
