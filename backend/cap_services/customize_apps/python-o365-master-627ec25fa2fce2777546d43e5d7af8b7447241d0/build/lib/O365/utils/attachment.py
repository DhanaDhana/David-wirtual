...
class BaseAttachment(ApiComponent):
    ...
    def attach(self, api_object, on_cloud=False):
        """ Attach this attachment to an existing api_ob__attachment_detail__attachment_detailject. This
...
class BaseAttachments(ApiComponent):
    ...
    def __init__(self, parent, attachments=None):
        ...
        self._parent = parent
        self.__attachments = []
        self.__attachment_detail = []
        self.__attachment_obj = []
    ...
    def detail_to_api_data(self):
        ...
        #     for at in self.__attachment_detail:
        #         if str(attachment) == str(at['attachment'][0]):
        #             toapi= {'id': at['attach_id'], 'attachment': attachment.to_api_data()}
        #             at_list.append(toapi)
        for at in self.__attachment_detail:
            toapi = {'id': at['attach_id'], 'attachment': at['attachment'][0].to_api_data()}
            at_list.append(toapi)
        return at_list

    def at_detail(self, attachments):
        return self.__attachment_detail
    ...
    def add(self, attachments):
        ...
        if attachments:
            ...
            if isinstance(attachments, (list, tuple, set)):
                ...
                attachments_temp = [
                    ...
                    for attachment in attachments]
                            ...
                            dict) and 'attach_id' in attachments:
                # User provided attachments

                attachments_temp = [
                    self._attachment_constructor(attachments['attachment'], parent=self)
                    ]
                ...
                attachments_objId = [{'id':attachment['id'],'name':self._attachment_constructor(
                    {self._cloud_data_key: attachment}, parent=self,
                    on_cloud=True)}
                    for attachment in
                    attachments.get(self._cloud_data_key, [])]
            ...
            at_detail = {'attach_id': attach_id, 'attachment': attachments_temp}
            self.__attachments.extend(attachments_temp)
            self.__attachment_detail.append(at_detail)
            self.__attachment_obj.append(attachments_objId)
            self._update_parent_attachments()
            self._track_changes()
